...
use self::indexing::{
    Blocks, ChainLengths, ExplorerVoteProposal, StakePoolBlocks, StakePoolMeta, Transactions,
    VotePlans,
};
use self::persistent_sequence::PersistentSequence;
use self::{
    error::{ExplorerError as Error, Result},
    indexing::Pristine,
};
...
pub type State = indexing::StateRef;
...
impl ExplorerDb {
    pub fn bootstrap(block0: Block) -> Result<Self> {
        let pristine = Pristine::new("explorer-db")?;
    ...
    pub async fn get_stake_pool_data(&self, pool: &PoolId) -> Option<Arc<StakePoolMeta>> {
        // let pool = pool.clone();
